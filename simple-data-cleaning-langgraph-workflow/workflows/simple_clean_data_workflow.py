...
def reasoning_node(state: DataState) -> DataState:
    ...
    prompt = (
        ...
        "If only misisng data then action should be 'clean_missing', if only outliers detected then action should be 'remove_outliers'.\n\n"
        "if both missing values and outliers are detected the action should be'both'.\n\n"
        f"{state['summary']}\n\n"
...
def handle_missing_values_and_remove_outliers(state: DataState) -> DataState:
    """Fill missing numeric values with the column mean and Remove outliers using IQR method."""
    update_one = remove_outliers(state)
    updated_state = {**state, **update_one}
    update_two = handle_missing_values(updated_state)
    updated_state = {**state, **update_two}
    update_three = remove_outliers(updated_state)
...
workflow.add_node("both", handle_missing_values_and_remove_outliers)

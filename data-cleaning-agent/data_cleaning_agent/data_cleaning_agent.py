...
def make_lightweight_data_cleaning_agent(
    ...
    def create_data_cleaner_code(state: GraphState):
        ...
        # NEW: Prompt follows user-selected instructions; previous fixed steps kept below as comments
        data_cleaning_prompt = PromptTemplate(
            template="""
            You are a Data Cleaning Agent. Create a {function_name}() function to clean the data.

            User Instructions (ONLY implement these steps — do not add extra cleaning):
            {user_instructions}
            ...
            Work on a copy of the input DataFrame and return only the cleaned DataFrame.
            """,
            input_variables=["user_instructions", "all_datasets_summary", "function_name"]
        )
        # PREVIOUS: Fixed basic cleaning steps were always included in the prompt
        ...
        # NEW: Default instructions match the previous basic steps when none are provided
        response = data_cleaning_agent.invoke({
            "user_instructions": state.get("user_instructions") or (
                "1. Drop columns with more than 40% missing values.\n"
                "2. Impute missing values for numeric columns only (mean or median); do not impute categorical columns.\n"
                "3. Remove duplicate rows."
            ),
            # PREVIOUS: "user_instructions": state.get("user_instructions") or "Follow the basic cleaning steps.",
